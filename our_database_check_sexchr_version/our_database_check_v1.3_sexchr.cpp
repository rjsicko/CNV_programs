...
using namespace std;
const string VERSION = "our_database_check_v1.3_sexchr";



//A struct for our CNVs (used for both the CNV file and the cumulative db file)
struct CNV{
	//cumulative DB format: Our_CNV_ID	Study_ID	Disorder	Case_or_Control	Chromosome	Start	End	Copy_Number	sex
	//GVAT format: ourID	subjectID	phenotype	case/control	chr	start	stop	copy_num sex
	string our_id, study_id, phenotype, chr, sex;
	int copy_num, case_or_control;	
	double start, stop;	//start and end points for each cnv
	
	string internal_type;	//store gain/loss type for sex chr m/f cnvs
	...
	//each CNV in our list
		...
		//for each cumulative db entry
			...
			//lets set our similar phenotype flag
				...
				//debugging
				/*if(me->our_id == "HPPE_Xc_0551")
					if(them->our_id == "EATEFKT_Xc_0619")
					{
						cout << "them internal type = " << them->internal_type << endl;
						cout << "me internal type = " << me->internal_type << endl;
					}*/
			//lets set our same cnv type flag
			//just use the internal_type
				bool cnv_same_flag = true;
				if(me->internal_type == them->internal_type)
				{
					cnv_same_flag = true;
				}
				else
...
int main()
{
	...
	cout << "ourID\tstudyID\tphenotype\tcase/control(1,2)\tchr\tstart\tstop\tcopy_num\tsex\n";
	...
	cout << "Generating internal CNV types\n";
	GenerateType(master_list_cnvs);
	GenerateType(comparison_list_cnvs);
