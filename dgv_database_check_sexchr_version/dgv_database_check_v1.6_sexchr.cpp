/*****************************************************
DGV_database_check_v1.6_sexchr
...
bool ReadFileCnv(string filename,vector<CNV>& our_cnvs)
{
	...
	while (getline(infile, line))
	{
		...
		string throw_away_phenotype;
		//ourID	sample	phenotype	case/control	chr	start	stop	copy_num	Isex
		ss >> dummy_cnv.our_id >> dummy_cnv.study_id >> throw_away_phenotype
			>> dummy_cnv.case_control >> dummy_cnv.chr >> dummy_cnv.start
	...
	for (vector<CNV>::iterator me = master_list_cnvs.begin();me != master_list_cnvs.end();me++)
	{
		...
		else if(me->sex == "F")
		{
			...
			if(me->chr == "X")
			{
				...
				if(me->copy_num == 2)
				{
					/*if(me->our_id == "EATEFKT_Xc_0619")
						cout << "we got to the set internal_type\n";*/
					me->internal_type = "normal";
				}
				if(me->copy_num == 3)
				{
		...
		//Debugging
		//cout << me->internal_type << endl;
		/*if(me->our_id == "EATEFKT_Xc_0619")
		{
			cout << me->sex << endl;
			cout << me->chr << endl;
			cout << me->copy_num << endl;
		}*/
		...
		for(vector<BLOCK>::iterator them = otherCNVs.begin();them != otherCNVs.end();them++)
		{
			if(me->chr == them->chr)
			{
				...
				{	//If there is some overlap with our CNV, then lets keep track of it
					...
					//DGV2 CNVs are only coded 1 or 3
					if( (me->internal_type == "gain" && them->copyNum == 3) || (me->internal_type == "loss" && them->copyNum == 1) )
					{
						//if we match gain to gain or loss to loss
		...
		for(vector<BLOCK>::iterator them = otherCNVs.begin();them != otherCNVs.end();them++)
		{
			...
			{//exact match
				...
				if(same_type)
				{
					...
					//an exact match so we can check if their freq is greater than ours
				...
				{//not exact match, but some overlap
					//cout << "them with overlap? " << them->our_id << "overlap = " << overlap_CNV << endl;
									
					if(same_type)
					{
						//if the overlap we are checking is greater or equal to our current overlap
						if( (overlap_CNV > me->overlap_single_same) || (overlap_CNV = me->overlap_single_same) )
						{//we have a new champ
							me->overlap_single_same = overlap_CNV;
							//lets check their freq and replace ours with theirs if theirs is higher
							if(me->champ_freq_same < them->freq)
								me->champ_freq_same = them->freq;
						}
					}
					...
					{
						if( (overlap_CNV > me->overlap_single_diff) || (overlap_CNV = me->overlap_single_diff) )
						{
...
int main()
{
	...
	GenerateType(master_list_cnvs);	//assign our internal type
