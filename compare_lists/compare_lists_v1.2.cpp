...
bool WriteLog(int num_cnvs, int num_regions)
{
	...
	outfile << "Program: compare_lists_v1.2\n";
	...
	outfile << "Type specific on?: " << type_specific << endl;
...
bool DefineConfig()
{
	ifstream infile("compare_lists_v1.2.config");
	if(!infile.is_open())
		return false;
